...
형식1] "형식문자열" % 값  형식
     ...
     만약 % 뒤의 값이 복수 개이면 튜플( ()로 묶인걸 의미)로 묶어주어야 한다.
     예]
     name ='이름은 %s입니다' % '김길동' -> %s에 해당하는게 '김길동'
     print(name)
...
형식2] "형식문자열".format(인자들) : str의 메소드
        형식문자열 내의 변환 지시자의 형식은 {숫자}, {이름} 이다
        ※ 숫자 사용시 숫자는 0부터 시작 그렇지 않으면 아래 에러 발생
        IndexError: tuple index out of range
    예]
     name ='이름은 {1}이고 나이는 {0}살입니다'.format(20,'김길동',) -> 1, 0 은 인덱스 숫자, 김길동이고 20살
     print(name)
     person = '이름:{name},나이:{age}'.format(name='김길동',age=20)
     print(person)
'''
kor = 99; eng = 80; math = 96
avg = (kor+eng+math)/3 #자바처럼 3.0으로 안해도 됨
print('[형식 문자열 미사용시]')
print('국어:'+str(kor)+',영어:'+str(eng)+',수학:'+str(math)+',평균:'+str(avg))
#국어:99,영어:80,수학:96,평균:91.66666666666667(자바 더블형인셈, 소수점 많음)

print('[형식 문자열 사용시 - 포매팅 연산자(%)]')
print('국어:%d,영어:%d,수학:%d,평균:%f' % (kor,eng,math,avg))
#국어:99,영어:80,수학:96,평균:91.666667(자바 플로트형인셈, 소수점 적음)
print('국어:%d,영어:%d,수학:%d,평균:%.14f' % (kor,eng,math,avg))
#형식문자열 미사용시 상황의 소수점과 동일하게 맞춰주기

print('[형식 문자열 사용시 - str타입의 format()메소드 : 숫자인덱스')#인덱스별로 값 들어감
print('국어:{0},영어:{1},수학:{2},평균:{3}'.format(kor,eng,math,avg))
#국어:99,영어:80,수학:96,평균:91.66666666666667

print('[형식 문자열 사용시 - str타입의 format()메소드 : 숫자인덱스 생략')#인덱스 생략시 순서대로 들어감
...
#순서 일치는 무관, 키값만 일치하면 됨

#형식문자열 쓰는 곳에서 특수문자(이스케이프문자) 사용가능
print('국어:%d\t영어:%d\t수학:%d\t평균:%f' % (kor,eng,math,avg))
print('국어:{}\t영어:{}\t수학:{}\t평균:{}'.format(kor,eng,math,avg))
...
print('[출력 자리수 미 지정]')
print('국어:%d,영어:%d,수학:%d,평균:%f' % (kor,eng,math,avg))
print('[출력 자리수 지정(양수)]')#오른쪽부터 x칸(.2f는 소수점 둘째자리 의미)
