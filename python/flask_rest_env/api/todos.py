...
class Todos(Resource):
    ...
    # get(self,매개변수)의 매개변수(todo_id)와 app.py의 add_resource(클래스명,'/todos/<변수명>')의 <변수명>이 일치해야 한다
    ...
    # 2. 키값(todo_id)에 따른 데이타 수정 api-'task'라는 파라미터명으로 할 일 내용 받는다
    def put(self,todo_id):
        ...
        # 받은 데이타로 수정
        #dict_ = args{'task': args['task'], 'when': args.get('when')}
        TODOS[todo_id] = args  # 덮어쓰기
        return make_response(TODOS,201)
