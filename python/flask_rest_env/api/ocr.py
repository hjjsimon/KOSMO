...
'''
서비스 계정 키를 사용하여 직접 인증하는 코드
google-auth, google-auth-oauthlib 라이브러리를 사용하여 서비스 계정 키를 로드
google-cloud-vision 라이브러리를 사용하여 Vision API에 접근
1.필요한 라이브러리 설치
pip install google-cloud-vision google-auth  google-auth-oauthlib
pip install --upgrade google-auth


base64인코딩 이미지 만드는 자스 소스
...
class OCR(Resource):
    def __init__(self):
        ...
        self.credentials_path='./api/lithe-vent-392501-59ebb081f29e.json'
    ...
    def post(self):
        ...
        print(args['base64'])
        texts=self.detect_labels(args['base64'])
        print(texts)
    ...
    def detect_labels(self,image_base64):
        ...
        객체 탐지 결과를 받는 함수
        image_base64:이미지의 BASE64인코딩 문자열
