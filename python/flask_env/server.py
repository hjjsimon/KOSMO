#Flask 웹 어플리케이션을 Waitress WSGI로 서비스 하기

'''
Flask 웹 어플리케이션 서비스 하기
1. 콘솔창에서 python server.py 명령어 실행
   (flask_env) D:\CCH\Workspace\Python\FlaskApp>python server.py
혹은
2. .exe실행파일 만들어서 실행
[.py를 .exe로 만들기]
1) pip install pyinstaller
2) pyinstaller --help 사용법 확인
   예 : pyinstaller --noconsole --onefile server.py -i="./favicon.ico" --add-data "templates;templates" --add-data "static;static"
   --noconsole옵션은 콘솔창을 띄우지 않기 위한 설정
  -i는 .exe파일의 아이콘 설정
  --onefile server.py는 server.py를 server.exe로 생성
   아이콘은 https://www.iconfinder.com/에서 이미지 다운후

    https://www.icoconverter.com에서 .ico로 컨버트
