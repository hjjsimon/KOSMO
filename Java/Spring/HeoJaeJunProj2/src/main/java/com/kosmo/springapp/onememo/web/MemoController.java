...
public class MemoController {
	...
	public String list(
			//@ModelAttribute("id") String id, //핸들러 인터셉터로 컨트롤러 넘기기 사전에 인증여부 판단시 주석
	...
	public String write(
		...
		//뷰정보 반환
		return "onememo/bbs/Write";
	}
	
	//입력처리]
	@PostMapping("/Write.do")
	public String writeOk(@ModelAttribute("id") String id,@RequestParam Map map, Model model) {
		...
		if(affected == 0) {//입력 실패 OneMemoServiceImpl 확인
			...
			return "onememo/bbs/Write"; //포워드 시키면 파라미터 유지되므로 param.xxx 그대로 값 유지됨
		}
		//뷰정보 반환 
	...
	public String editOk(
		...
		if(affected == 0) {//입력 실패 OneMemoServiceImpl 확인
			...
			return "onememo/bbs/Edit"; //포워드 시키면 파라미터 유지되므로 param.xxx 그대로 값 유지됨
		}
		//뷰정보 반환 
		return "forward:/onememo/bbs/View.do";
	}
