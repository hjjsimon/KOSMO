...
public class PagingUtil {
	...
	public static String pagingText(int totalRecordCount,int pageSize,int blockPage,int nowPage,String page){
		...
		//처음 및 이전을 위한 로직
		if(intTemp != 1){
                		//NOWPAGE로 설정하면 호버시 왼쪽하단에 생기는 링크가 이쁘게 뜸
	...
	/**
	 * 총 레코드 수/페이지사이즈 수/블록페이지 수/현재페이지번호 가 
	 * 설정되어있는 맵(검색시에는 searchWord/searchColumn 키도 존재함)을 인자로 받아서 연산을 통해
	 * 시작/끝 행번호 및 총 페이지 수를 맵에 저장하는 메소드
	 * 
	 * @param map	위 4개의 값이(총 레코드 수/페이지사이즈 수/블록페이지 수/현재페이지번호) 저장된 맵
	 */
	public static void setMapForPaging(Map map) {
		//페이징을 위한 로직 시작
		//전체 레코드 수
		int totalRecordCount = Integer.parseInt(map.get(TOTAL_RECORD_COUNT).toString());//object 반환함, toString 후 int로
