...
<jsp:include page="/template/Top.jsp"/><!-- Top에 taglib넣어도 안됨, 인클루드는 바뀐 후 들어오는거라 -->
    <div class="container" style="margin-top:50px">
        <div class="jumbotron bg-info">
            <h1>자료실 작성 <small>자료 파일을 등록하세요</small></h1>            
        </div><!--jumbotron-->
        ...
        <form method="post" action="<c:url value="/DataRoom/Write.kosmo"/>" enctype="multipart/form-data">
			<div class="form-group">
				<label><kbd class="lead">올린이</kbd></label> <input type="text"
					class="form-control" placeholder="이름을 입력하세요" name="name" value="${ empty param.name ?"": param.name}">
				<!-- 처음 요청시 param 비어있음, 그러면 빈문자열 나오게함 -->
			</div>
			<div class="form-group">
				<label><kbd class="lead">제목</kbd></label> <input type="text"
					class="form-control" placeholder="제목을 입력하세요" name="title" value="${ empty param.title ?"": param.title}">
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<label><kbd class="lead">비밀번호</kbd></label> <input type="password"
					class="form-control" placeholder="비밀번호를 입력하세요" name="password" value="${ empty param.password ?"": param.password}">
			</div>
