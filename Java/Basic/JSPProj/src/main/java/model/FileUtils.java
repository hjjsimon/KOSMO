...
public class FileUtils {
	 ...
	 * @return
	 */
	public static String getNewFileName(String path, String fileName) {//path-> 업로드 디렉토리
		// fileName=원격.txt
	...
	/**
	 * 다중 파일 업로드용 메소드로 파일을 하나라도 업로드시 파일명이 저장된 StringBuffer를 반환
	 * 미업로드시 null 반환하도록함
	 * 
		...
		try {			
			for(Part part:parts) {
				if(part.getContentType() !=null) {//input type="file"인 경우				
					...
					//파일명을 스프링버퍼에 저장
					fileNames.append(systemFileName+",");//구분자 , 추가 내맘
