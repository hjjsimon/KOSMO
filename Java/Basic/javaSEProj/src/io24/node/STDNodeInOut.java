...
//IO, 파일에 데이터를 저장, 파일에 저장된 데이터를 자바가 읽어와서 프로그램에서 출력해줌->자바IO교안 참고
	...
	//인풋스트림과 소스, 인풋스트림과 프로그램, 아웃풋스트림과 프로그램, 아웃풋스트림과 데이터목적지 -> 4개 노드 존재, 노드로 구성된 인풋,아웃풋스트림을 노드스트림이라고함
...
===========================+========================+===========================+==========================+===========================
			   	  |                        |                           |                          |
	추상클래스       |   InputStream          |   OutputStream            |       Reader             |      Writer
                  |                        |  (PrintStream이 자식)       |                          |
-------------------------- +------------------------+---------------------------+--------------------------+---------------------------- 
...
---------------------------+------------------------+---------------------------+--------------------------+----------------------------
			                                						   |		                   |
Bridge Stream(바이트를 문자로) 			                                   |	InputStreamReader      |      OutputStreamWriter
									                                   |                           |
---------------------------+------------------------+---------------------------+--------------------------+-----------------------------
	              |                        |                           |                           |
Filter Stream     |   DataInputStream      |     DataOutputStream      |       BufferedReader      |       BufferedWriter
(필터마다 용도다름)   |                        |                           |                           |
(필터거쳐 ㅇ->ㅁ됨)  |   BufferedInputStream  |     BufferedOutputStream  |                           |       PrintWriter(프린트니까 읽는read는없음)
                  |                        |                           |                           |
...
키보드에 직접 연결할 수 있는 바이트 기반의 노드스트림(입력 스트림):System.in*****(암기)->시스템클래스의 멤버가 in, InputStream 타입
...
public class STDNodeInOut {
	...
	public static void main(String[] args) throws IOException {
		...
		//out.close();//스트림 닫기, write로 스트림에 A가 들어감, close는 스트림을 뺀다는것, 그러면 스트림에 있던게 자동 flushing됨-> 관을 빼서 다 모니터로 나온다는뜻->close하면 자동으로 flush되지만 그래도 에러대비 해줘야함
...
/*		while((ascii=is.read()) !=13) {//\r 읽으면 while 빠져나옴
		 ...
		 */
		//필터효과 적용]계란판에 담고 내보냄->필터는 아니지만 byte[], char[]를 사용, buffer cache RAM HDD -> 이런게 메모리, buffer 써서 빨라진거래
